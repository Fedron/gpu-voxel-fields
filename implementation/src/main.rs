...
impl Bounds {
    fn contains(&self, position: glam::UVec3) -> bool {
...
impl Node {
    ...
    /// Inserts a new voxel at the position specified.
    fn insert(&mut self, position: glam::UVec3, min_resolution: u32, voxel: Voxel) {
        ...
        match self.ty {
            ...
            NodeType::Leaf(voxel) => {
                let mut branch: [Box<Option<Node>>; 8] = core::array::from_fn(|_| Box::new(None));
                for i in 0..8 {
                    if i != child_info.octant as u8 {
                        ...
                        new_node.ty = NodeType::Leaf(voxel.clone());
            ...
            NodeType::Branch(ref mut branch) => {
                ...
                child_node.insert(position, min_resolution, voxel);
            ...
            NodeType::Empty => {
                ...
                new_node.insert(position, min_resolution, voxel);
    ...
    fn compact(&mut self) {
        ...
        println!("Compacting node with bounds: {:?}", self.bounds);
        ...
        for i in 0..8 {
            if let Some(child_node) = children[i].deref() {
                match &child_node.ty {
